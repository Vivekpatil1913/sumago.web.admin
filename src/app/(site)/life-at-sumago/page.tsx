...
import { cultureGalleryImages, eventGalleries } from "@/lib/preview-assets";
...
const cultureGallery = cultureGalleryImages.map((src, i) => ({
  src,
  alt: `Sumago team, engineers, and workspace — culture still ${i + 1}`,
}));

const eventCategories = eventGalleries.map((c) => ({
  key: c.key,
  title: c.title,
  images: c.images.map((src, i) => ({ src, alt: `Sumago ${c.title} — photo ${i + 1}` })),
}));
  ...
  return (
    <>
      <PageHero
        ...
        title={<>Life at Sumago: <span className="text-metal-red">curiosity and craft</span>.</>}
      ...
      <Section>
        <SectionHeading
          eyebrow="Our culture"
          title={<>Where great work and <span className="text-metal-red">good people</span> meet.</>}
        ...
        <MosaicGallery images={cultureGallery} label="Life at Sumago photo gallery" className="mt-12" />
      </Section>

      <Section muted>
        <SectionHeading
          eyebrow="What it's like"
          title={<>The things that shape <span className="text-metal-red">how we work</span>.</>}
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {culturePillars.map((p, i) => (
            ...
            >
              ...
              <p className="relative mt-2 text-sm leading-relaxed text-ink/65">{p.desc}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section dark>
        <SectionHeading
          ...
          title={<>Life at Sumago, <span className="text-metal-red">off the clock too</span>.</>}
          ...
          <div className="relative grid items-center gap-10 p-8 sm:p-10 md:grid-cols-[1.15fr_1fr] md:p-14">
            ...
            <div>
              ...
              <h2 className="mt-5 text-3xl font-bold leading-[1.1] tracking-tight text-ink sm:text-4xl lg:text-5xl">
                Want to build <span className="text-metal-red-shine">with us?</span>
              </h2>
              <p className="mt-4 max-w-md text-base leading-relaxed text-ink/65">
                Curiosity, ownership, and high standards count for more than a perfect
                CV here — engineers, designers, and delivery leads who want the work to
                matter tend to stay a long while.
              </p>
                ...
                {openPositions.slice(0, 3).map((p) => (
                  <li key={p.slug}>
                    ...
                    >
                      ...
                      <ArrowRight
                        size={16}
                        className="shrink-0 text-ink/30 transition-all duration-200 group-hover:translate-x-0.5 group-hover:text-brand"
                      />
                    </Link>
                  </li>
                ))}
