...
/** A soft brand glow blob, positioned + animated via classes. */
...
const PARTICLES = [
  { left: "12%", top: "28%", size: 5, dur: 9, color: "rgba(255,120,90,0.9)" },
  { left: "26%", top: "66%", size: 4, dur: 11, color: "rgba(255,255,255,0.8)" },
  { left: "44%", top: "20%", size: 6, dur: 10, color: "rgba(215,52,56,0.95)" },
  { left: "60%", top: "74%", size: 4, dur: 12, color: "rgba(255,255,255,0.7)" },
  { left: "75%", top: "32%", size: 5, dur: 9.5, color: "rgba(255,120,90,0.85)" },
  { left: "88%", top: "56%", size: 4, dur: 11.5, color: "rgba(255,255,255,0.7)" },
  { left: "9%", top: "78%", size: 4, dur: 13, color: "rgba(215,52,56,0.8)" },
...
export function HeroEffect({
  ...
  variant: HeroVariant;
  /** Scales every brand-red layer (tint + wash + variant glows). 1 = full,
   *  lower = subtler. Use for heroes that want a near-neutral dark backdrop. */
  redOpacity?: number;
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      {/* All brand-red layers grouped so their intensity can be scaled together. */}
      <div
        className="absolute inset-0"
        style={redOpacity !== 1 ? { opacity: redOpacity } : undefined}
      >
        {/* Base cinematic tint shared by every variant. */}
        <div className="absolute inset-0 bg-[radial-gradient(60%_60%_at_50%_0%,rgba(215,52,56,0.16),transparent_70%)]" />
        {/* Animated red gradient wash — always-on brand-red drift for every hero. */}
        <div className="fx-red-aurora absolute inset-0" />
        {renderVariant(variant)}
      </div>
      {/* Neutral depth glow — restores subtle dimension when red is dialed down
          (mirrors the homepage hero's white glow). */}
      {redOpacity < 1 ? (
...
function renderVariant(variant: HeroVariant) {
  switch (variant) {
    /* Drifting aurora blobs — about. */
    case "aurora":
      return (
        <>
          <Blob className="-left-24 top-0 h-80 w-80 animate-[blob-float_11s_ease-in-out_infinite] bg-brand/25" />
          <Blob className="right-0 top-10 h-72 w-72 animate-[blob-float_14s_ease-in-out_infinite_reverse] bg-[#7a1519]/30" />
          <Blob className="left-1/3 -bottom-16 h-72 w-72 animate-[blob-float_17s_ease-in-out_infinite] bg-[#ff6b4a]/15" />
        </>
      );

    /* Engineering grid + brand glows — solutions. */
    case "blueprint":
      return (
        <>
          <div className="fx-grid-static absolute inset-0" />
          <Blob className="left-1/2 -top-24 h-80 w-[36rem] -translate-x-1/2 animate-[blob-float_13s_ease-in-out_infinite] bg-brand/20" />
          <Blob className="-right-20 bottom-0 h-64 w-64 animate-[blob-float_16s_ease-in-out_infinite_reverse] bg-[#1e83f0]/10" />
        </>
      );

    /* Grid scrolling upward — careers. */
    case "grid":
      return (
        <>
          <div className="fx-grid absolute inset-0" />
          <Blob className="left-1/4 top-0 h-72 w-72 animate-[blob-float_12s_ease-in-out_infinite] bg-brand/20" />
        </>
      );

    /* Synthwave perspective floor — industries detail. */
    case "floor":
      return (
        <>
          <div className="fx-floor absolute inset-0" />
          <Blob className="left-1/2 top-6 h-72 w-[34rem] -translate-x-1/2 animate-[blob-float_13s_ease-in-out_infinite] bg-brand/25" />
        </>
      );
    ...
    case "dots":
      return (
        <>
          ...
          <Blob className="right-1/4 top-0 h-72 w-72 animate-[blob-float_14s_ease-in-out_infinite_reverse] bg-brand/18" />
        </>
      );
    ...
    case "streaks":
      return (
        <>
          ...
          <Blob className="-left-16 top-4 h-72 w-72 animate-[blob-float_15s_ease-in-out_infinite] bg-[#ff6b4a]/18" />
          <Blob className="right-0 bottom-0 h-64 w-64 animate-[blob-float_18s_ease-in-out_infinite_reverse] bg-brand/18" />
        </>
      );
    ...
    case "starfield":
      return (
        <>
          ...
          <Blob className="left-1/2 -top-20 h-72 w-[32rem] -translate-x-1/2 animate-[blob-float_15s_ease-in-out_infinite] bg-brand/18" />
        </>
      );
    ...
    case "spotlight":
      return (
        <>
          <div className="fx-grid-static absolute inset-0 opacity-60" />
          <div className="absolute left-1/2 top-[40%] -ml-[20rem] -mt-[20rem] h-[40rem] w-[40rem] animate-[hero-sweep_9s_ease-in-out_infinite] rounded-full bg-brand/25 blur-[130px]" />
        </>
      );
        ...
        <div className="absolute inset-0 grid place-items-center">
          <div className="relative h-[34rem] w-[34rem]">
            ...
            <div className="absolute inset-0 animate-[hero-spin_26s_linear_infinite]">
              <span className="absolute left-1/2 top-0 h-2.5 w-2.5 -translate-x-1/2 rounded-full bg-brand shadow-[0_0_16px_4px_rgba(215,52,56,0.6)]" />
            </div>
            <div className="absolute inset-[18%] animate-[hero-spin-rev_20s_linear_infinite]">
              <span className="absolute left-1/2 top-0 h-2 w-2 -translate-x-1/2 rounded-full bg-[#ff6b4a] shadow-[0_0_14px_3px_rgba(255,107,74,0.55)]" />
            </div>
            ...
            <div className="absolute inset-0 grid place-items-center">
              <div className="h-24 w-24 rounded-full bg-brand/25 blur-2xl" />
            </div>
          </div>
        </div>
      );
        ...
        <div className="absolute inset-0 grid place-items-center">
          <div className="relative grid h-96 w-96 place-items-center">
            {[0, 1, 2, 3].map((i) => (
              <span
                key={i}
                className="absolute h-96 w-96 rounded-full border border-brand/40 animate-[hero-ring_4s_ease-out_infinite]"
                style={{ animationDelay: `${i}s` }}
              />
            ))}
            <span className="h-2.5 w-2.5 rounded-full bg-brand shadow-[0_0_18px_5px_rgba(215,52,56,0.6)]" />
          </div>
        </div>
      );
    ...
    case "constellation":
      return (
        <>
          ...
          <Blob className="left-1/2 -top-16 h-64 w-[30rem] -translate-x-1/2 animate-[blob-float_14s_ease-in-out_infinite] bg-brand/16" />
        </>
      );
    ...
    case "circuit":
      return (
        <>
          ...
          <Blob className="-right-16 top-0 h-72 w-72 animate-[blob-float_15s_ease-in-out_infinite_reverse] bg-brand/18" />
        </>
      );
  }
}
